...
class Sales extends MY_Controller
{
    ...
    function add($sale_order_id = NULL, $delivery_id = NULL,$quote_ID = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if($invoice_view == 0){
                redirect("sales/print_w/" . $s->row()->id);
            }
...
function invoice_concrete_angkor($id=null)
{
    ...
    function getDigitalData($id){
        $digital_items 	= $this->sales_model->getPaidDigital($id);
        $standard_items = $this->sales_model->getFreeDigital($id);

        $str_arr 		= array();
        $dig_arr		= array();
        if($digital_items){
            foreach ($digital_items as $ditems) {
                $small_price = $ditems->price/$ditems->qty_balance;
                $di_variant  = $this->site->getProductVariants($ditems->product_id);
                $i = 1;
                $divide = 0;
                foreach ($di_variant as $dvar) {
                    if ($i == 1) {
                        $divide = $ditems->qty_balance;
                    }
                    $dig_qty = intval($divide/$dvar->qty_unit);
                    $divide  = $divide%$dvar->qty_unit;

                    $dig_arr[] = array(
                        'name' 		=> $ditems->product_name,
                        'quantity' 	=> $dig_qty,
                        'variant' 	=> $dvar->name,
                        'price' 	=> ($small_price * $dvar->qty_unit),
                    );
        ...
        if($standard_items){
            foreach ($standard_items as $sitems) {
                $small_price = $sitems->price/$sitems->qty_balance;
                $di_variant  = $this->site->getProductVariants($sitems->product_id);
                $i = 1;
                $divide = 0;
                foreach ($di_variant as $dvar) {
                    if ($i == 1) {
                        $divide = $sitems->qty_balance;
                    }
                    $dig_qty = intval($divide/$dvar->qty_unit);
                    $divide  = $divide%$dvar->qty_unit;

                    $str_arr[] = array(
                        'name' 		=> $sitems->product_name,
                        'quantity' 	=> $dig_qty,
                        'variant' 	=> $dvar->name,
                        'price' 	=> ($small_price * $dvar->qty_unit),
                    );
        ...
        $digital_items = array_merge($dig_arr, $str_arr);
        //$this->erp->print_arrays($digital_items);
    ...
    {
        $digitalDatas = $this->getDigitalData($id);
