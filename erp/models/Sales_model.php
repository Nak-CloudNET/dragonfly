...
class Sales_model extends CI_Model
{
    ...
    public function getPaidDigital($sale_id = null)
    {
        $this->db->select('erp_sale_items.id, sale_id, digital_id, product_id,
							(
								CASE
								WHEN erp_products.`name` IS NULL THEN
									erp_sale_items.product_name
								ELSE
									erp_products.`name`
								END
							) AS product_name,
							SUM(quantity_balance) as qty_balance,
							SUM(subtotal) as price'
        )
            ->from('sale_items')
            ->join('products', 'products.id = sale_items.digital_id', 'left')
            ->where(array('sale_id'=>$sale_id, 'unit_price >' => 0))
            ->group_by('digital_id, (
								CASE
								WHEN erp_products.`name` IS NULL THEN
									erp_sale_items.product_name
    ...
    public function getFreeDigital($sale_id = null)
    {
        $this->db->select('erp_sale_items.id, sale_id, digital_id, product_id,
							(
								CASE
